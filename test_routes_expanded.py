...
class ExpandedRouteCoverageTestCase(unittest.TestCase):
    ...
    def test_submit_lead_capture_duplicate_does_not_email_admin(self):
        # Duplicate submissions must NOT trigger another admin notification —
        # otherwise an attacker rotating IPs could flood the inbox even with
        # the per-IP rate limit, since the storage layer silently no-ops on
        # repeats. The endpoint still returns success to avoid leaking which
        # emails are already pending.
        with patch.object(
